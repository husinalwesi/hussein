<template>
  <main v-if="!isLoading" class="corporate-group">
    ...
    <template v-else-if="data">
        ...
        >
          <template #section-content>
            ...
            >
              <h1 class="title">
                {{
                  data && data.hero && data.hero.title ? data.hero.title : title
                }}
              </h1>
      ...
      <SectionContentWrapper
        ...
        section-wrapper-classes="mt-[-35px] mb-16 md:mb-20"
      >
        <template #section-content>
      ...
      <SectionContentWrapper
        ...
        "
        section-wrapper-classes="mb-10 md:mb-20"
      ...
      <template
        v-if="
          data.must_visit_section &&
            data.must_visit_section.achievements &&
            data.must_visit_section.achievements.length
        "
      >
        <SectionContentWrapper
        ...
        >
          <template #section-content>
            <Achievements :data="data.must_visit_section" />
          </template>
        </SectionContentWrapper>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      isLoading: false,
      data: null,
      title: "Corporate group packages",
      selectedTabDetails: {},
    };
  },
  async fetch() {
    try {
      ...
      const res = await this.$axios.$get(endPoints.GET_CORPORATE);
      // TODO: show no data msg when there's no data found.
      if (res.Success && res.Result) {
        this.data = res.Result;

        if (res.Result.categories && res.Result.categories.length) {
          const flattendArr = res.Result.categories.filter(
            (category) => !!category.title
          );
          this.data.categories = flattendArr;
          this.selectedTabDetails = flattendArr[0];
        }
      }
  ...
  head() {
    return this.$seoGenerator(this.data, `Corporate Group`)
  },
};
</script>

<style lang="scss" scoped>
@import "./corporate-group.scss";
</style>
