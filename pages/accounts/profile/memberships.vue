<!--TODO: combine this page with personal details page and make them as one page with multiple routes-->
<template>
...
export default {
  ...
  head() {
    return this.$seoGenerator({}, this.$t("PAGES_TITLE.ACCOUNTS.PROFILE.MEMBERSHIPS"));
  },
  computed: {
  ...
  mounted() {
    ...
    this.scrollToMemberships = setTimeout(() => {
      gsap.to(window, {
        duration: 0.667,
        scrollTo: { y: "#membership-card", offsetY: 100 },
      });
    }, 250);
  },
  beforeDestroy() {
    this.scrollToMemberships = null;
  },
  methods: {
    async getUsers() {
      try {
