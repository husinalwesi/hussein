            ...
            <template v-else>
              ...
              <template v-else>
                <UserPassport
                  ...
                  passport-locale-type="SNOWBOARD_PASSPORT"
    ...
    >
      <template #card-content>
        ...
        <template v-else>
          ...
          >
            <PassportAssessments
              :title="$t('ACCOUNT_PAGE.SKI_SCHOOL.PASSPORT_ASSESSMENTS.SNOWBOARD_PASSPORT')"
              :selected-user-details="selectedUserDetails"
              :is-book-btn-enabled="loaders.getLesson ? false : $isEmptyObject(selectedUserDetails.nextLesson || {})"
              @bookLessonEmiter="bookLessonEmitHandler"
              @fetchData="getUserPassport"
            />
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      selectedUserDetails: {},
      passportHeaderData: {
        passportType: "Snowboard",
        description: this.$t("ACCOUNT_PAGE.SKI_SCHOOL.SNOWBOARD_PASSPORT.NO_PREVIOUS_PASSPORT_DESCRIPTION")
      },
      loaders: {
        getUsers: false,
        getLesson: false,
  ...
  head() {
    return this.$seoGenerator({}, this.$t("PAGES_TITLE.ACCOUNTS.SKI_SCHOOL.SNOWBOARD_PASSPORT"));
  },
  computed: {
  ...
  methods: {
    ...
    bookLessonEmitHandler(data = {}) {
      ...
      this.$router.push(
        this.localePath(
          `/snowboard-lessons?level=${data.level}&lesson=0&age=${this.selectedUserDetails.ageEnum}`
        )
      );
    },
    async getUserNextLesson() {
      try {
        ...
        const { data: res } = await this.$axios.get(
           `${process.env.SHARED_ENVS.APIGEE_BASE_URL}${customLayerEndPoints.GET_NEXT_LESSON}?type=${lookups.PASSPORT_TYPE[1].id}`,
          {
            headers: {
    ...
    async getUserPassport() {
      try {
        ...
        const { data: res } = await this.$axios.get(
          `${process.env.SHARED_ENVS.APIGEE_BASE_URL}${customLayerEndPoints.GET_PASSPORT}/${this.selectedUserDetails.id}/${lookups.PASSPORT_TYPE[1].id}`
        );
        ...
        if (res && res.data) {
          ...
          if (isUserCompletedLessonAssessments) {
            this.passportHeaderData = {
              passportType: "Snowboard",
              description: this.$t("ACCOUNT_PAGE.SKI_SCHOOL.SNOWBOARD_PASSPORT.PREVIOUS_PASSPORT_DESCRIPTION"),
            };
          }
    ...
    toggleSelectedUser(user) {
      this.selectedUserDetails = {
        ...
        userAK: user.user_ak,
        snowPassportID: user.snowPassportID,
        ageEnum: this.getAgeEnum(user.dateOfBirth, "snowboard"),
      ...
      try {
        ...
        if (res && res.users && res.users.length) {
          const flattendUsersList = res.users.filter(
            (user) => moment().diff(user.dateOfBirth, "years") >= 8
          );
