<template>
  <div id="snowBG" ref="snowBg" />
</template>

<script>
import {ShaderProgram} from "@/assets/js/ShaderProgram.js";
import gsap from "gsap";

export default {
  name: 'OmanSnow',
  mounted() {
    this.$nextTick(() => this.buildFlakeLayer());
  },
  methods: {
    buildFlakeLayer() {
      ...
      const isMobile = this.$globalData.isMobileDevice;

      const alphaValue = isMobile ? 0.6 : 0.1;
      const fullAlpha = alphaValue + Math.random() * 0.4;
      ...
      const count = this.$globalData.isDesktopDevice ? 30000 : 10000;
      const reducedCount = count * 0.15;
      const snowFlakeSizeVal = isMobile ? "40.0" : "100.0";

      new ShaderProgram(this.$refs['snowBg'], {
        depthTest: false,
        texture: snowflake,
        uniforms: {
    ...
    void main() {
      ...
      gl_PointSize = ( a_size / gl_Position.w ) * ${snowFlakeSizeVal};
    ...
    void main() {
      ...
      gl_FragColor = vec4(snowflake.rgb, snowflake.a * v_color.a * u_alpha);
        ...
        onResize(w, h, dpi) {
          ...
          Array.from({length: w / h * count}, (snowflake, index) => {
            position.push(
            ...
            color.push(
              ...
              1,
              1,
              index < reducedCount ? customAlpha.odd : customAlpha.even
            )
        ...
        onUpdate(delta) {
          ...
          wind.current += wind.force * (delta * speedFactor.value)
          ...
          for (let i = 0; i < (w / h * count); i++) {
            newColors.push(
              ...
              1,
              1,
              i < reducedCount ? customAlpha.odd : customAlpha.even
            )
          }

          this.buffers.color = newColors
      ...
      document.addEventListener('scroll', () => {
        this.decelerate(speedFactor, customAlpha)
      }, {once: true});

      let eventType = this.$globalData.isMobileDevice ? 'touchmove' : 'mousemove'
    ...
    decelerate(speedFactor, customAlpha) {
      gsap.to(
        speedFactor,
        {
          value: 0.2,
