              ...
              <template #card-content>
                ...
                <template v-else>
                  ...
                  <div v-if="!isLoading" class="card-body">
                    <div v-for="(mainTicket, mainIndex) in orderData.collections" :key="mainIndex+refreshId"
                         class="mb-[32px]"
                    >
                      <div
...
export default {
  name: "AddGuest",
  components: {BaseDate, Breadcrumbs, Plus, NoDataFound},
  data() {
    return {
      isError: false,
      userDetails: {},
  ...
  methods: {
    ...
    selectDate(event) {
      this.$nextTick(() => {
        this.formData = Object.assign({}, this.formData, {
          date: moment(event.selectedDate.toString()).format(AppConfig.dateFormat),
        });
      });
    },
      ...
      if (this.orderData.collections) {
        this.orderData.collections.forEach((item) => {
          ...
          if (isCamps.length > 0) {
            ...
            item.tickets = _.uniqBy(item.tickets, 'camp_group_id')
          }
        });
    ...
    async save() {
      ...
      for (let i = 0; i < this.orderData.collections.length; i++) {
        ...
        dates.push(moment(vivaTicket.event_date).toISOString())
